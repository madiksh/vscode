...
if [ "$npm_config_arch" == "x64" ]; then
  if [ "$(echo "$@" | grep -c -- "--only-remote")" -eq 0 ]; then
    ...
    curl -s https://raw.githubusercontent.com/chromium/chromium/126.0.6478.36/tools/clang/scripts/update.py | python - --output-dir=$PWD/.build/CR_Clang --host-os=linux
    ...
    # https://source.chromium.org/chromium/chromium/src/+/refs/tags/126.0.6478.36:build/config/arm.gni
    # https://source.chromium.org/chromium/chromium/src/+/refs/tags/126.0.6478.36:build/config/compiler/BUILD.gn
    # https://source.chromium.org/chromium/chromium/src/+/refs/tags/126.0.6478.36:build/config/c++/BUILD.gn
